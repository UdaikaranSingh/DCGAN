...
if cuda_available:
	...
	if os.path.exists("enocder_model_chair.pth"):
		...
		classifier.load_state_dict(torch.load("classifier_model_chair.pth"))
		discriminator.load_state_dict(torch.load("discriminator_model_chair.pth"))
else:
	...
	if os.path.exists("encoder_model_chair.pth"):
		print("Loading in Model")
		encoder.load_state_dict(torch.load("encoder_model_chair.pth"), map_location='cpu')
		decoder.load_state_dict(torch.load("decoder_model_chair.pth"), map_location=torch.device('cpu'))
		classifier.load_state_dict(torch.load("classifier_model_chair.pth"), map_location=torch.device('cpu'))
		discriminator.load_state_dict(torch.load("discriminator_model_chair.pth"), map_location=torch.device('cpu'))
...
np.save("output_chairs.npy", np.array(output))